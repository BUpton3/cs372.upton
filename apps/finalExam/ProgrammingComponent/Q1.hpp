...
public:
    ...
    void internalNodeCount() {
        int internalNodes;
        internalNodes = totalInternalNodes(root);
        cout << "Total number of internal nodes: " << internalNodes << endl;
    }
    int totalInternalNodes(Node<T>* root) {
        int count = 0;
        queue<Node<T>*> q;
        q.push(root);
        while (!q.empty()) {
    ...
    void externalPath() {
        int sum;
        sum = externalPathLength(root);
        cout << "External Path Length: " << sum << endl;
    }
    int externalPathLength(Node<T>* root) {
        if (root == NULL)
            return 0;
        if (root->left == nullptr && root->right == nullptr) {
            return root->data;
        }
        return externalPathLength(root->left) + externalPathLength(root->right);
    }
};
